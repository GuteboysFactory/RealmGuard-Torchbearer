...
export function getM5Status() {
  ...
  return Object.freeze({
    phase: "M5",
    buildScope: "GEAR_INVENTORY_CONFLICT_TOOL_SERVICE_FOUNDATION",
...
function diagnosticsHtml() {
  ...
  return `<div class="realm-guard" style="padding:8px 12px;max-height:64vh;overflow:auto;">
    ...
    <p>qa.5 introduces profile-aware CORE services beneath the existing Inventory and Conflict UX. The services can inspect and validate the current data, but they do not write gameplay state yet.</p>
    <h3>Services</h3>
    <p>${s.services.join(" · ")}</p>
    <h3>Active policy</h3>
    ...
    <code>game.realmGuard.core.m5.inventory.validateZone(actor, gearId, "right-hand")</code><br>
    <code>game.realmGuard.core.m5.conflictTools.list(actor, { conflictType: "fight" })</code>
  </div>`;
}
...
export function installM5CoreServices() {
  ...
  Hooks.once("ready", () => {
    runtime = createM5Services(getActiveRulesProfile());
    exposeApi();
    console.log("realm-guard | CORE M5 Gear / Inventory / Conflict Tool services ready", getM5Status());
  });
}
