...
function maneuverHtml(state) {
  ...
  const choices = [];
  if (points >= 1) choices.push(`<button class="rg-conflict-primary ${side}" data-maneuver-choice="impede" data-rg-help="Spend 1 Maneuver margin to Impede the opponent: the opponent takes −1D on its next action." data-rg-help-title="Maneuver · Impede">Impede · opponent −1D next action</button>`);
  if (points >= 2) choices.push(`<button class="rg-conflict-primary ${side}" data-maneuver-choice="position" data-rg-help="Spend 2 Maneuver margin to Gain Position: your side gains +2D on its next action." data-rg-help-title="Maneuver · Gain Position">Gain Position · +2D next action</button>`);
  if (points >= 3) {
    choices.push(`<button class="rg-conflict-primary ${side}" data-maneuver-choice="disarm" data-rg-help="Spend 3 Maneuver margin to Disarm: disable an opposing weapon or appropriate Gear for the conflict." data-rg-help-title="Maneuver · Disarm">Disarm · disable weapon / gear</button>`);
    choices.push(`<button class="rg-conflict-primary ${side}" data-maneuver-choice="combo" data-rg-help="Spend 3 Maneuver margin to combine Impede and Gain Position: opponent −1D next action and your side +2D next action." data-rg-help-title="Maneuver · Combined Effect">Impede + Gain Position</button>`);
  }
...
function controlsHtml(state) {
  ...
  if (state.stage === "disposition") {
    const gmButton = !state.gm.disposition.rolled && game.user?.isGM ? `<button class="rg-conflict-primary gm" data-conflict-action="disposition" data-side="gm" data-rg-help="Generate the GM / Opposition Starting Disposition. The GM chooses the disposition Method before any roll is made." data-rg-help-title="GM Starting Disposition"><i class="fa-solid fa-dice"></i> Roll GM Disposition</button>` : "";
    const rangerButton = !state.ranger.disposition.rolled && canCaptainControl(state) ? `<button class="rg-conflict-primary ranger" data-conflict-action="disposition" data-side="ranger" data-rg-help="Roll the Rangers’ Starting Disposition using the conflict’s legal Skill and Base Ability procedure." data-rg-help-title="Ranger Starting Disposition"><i class="fa-solid fa-dice"></i> Roll Ranger Disposition</button>` : "";
