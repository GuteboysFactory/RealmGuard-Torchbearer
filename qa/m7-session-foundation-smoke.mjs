...
must(shadow.includes('mode: "SHADOW_READ_ONLY"'), "M7 must remain shadow/read-only in M7");
must(shadow.includes('authority: "LEGACY_MIXED"'), "Legacy Mixed must remain live authority in M7");
must(shadow.includes("liveApplication: false"), "M7 M7 must not apply CORE live");
...
must(manifest.version === "1.9.0-M7", `Unexpected manifest version: ${manifest.version}`);
